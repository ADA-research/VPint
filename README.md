# VPInt
This repository contains the code required to run VPint (value propagation-based spatial interpolation). However, for the time being, this repository contains only a static version of the code associated with our final paper submission to DAMI. For up-to-date code that is actively undergoing further development, particularly adding functionality relevant for remote sensing applications, please see the repository at https://github.com/LaurensArp/VPint . 

Although working code is available for spatio-temporal interpolation, we recommend spatial interpolation as its preferred use case. The spatio-temporal code is at this point outdated, and its support may get dropped entirely in the future.
